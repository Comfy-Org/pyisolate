...
class TestSealedWorkerSandboxLaunch:
    @patch("pyisolate._internal.host.subprocess.Popen")
    def test_sealed_worker_launches_via_bwrap(self, mock_popen: MagicMock) -> None:
        ext = _make_extension()
    ...
    @patch("pyisolate._internal.host.subprocess.Popen")
    def test_sealed_worker_uses_explicit_env_allowlist(self, mock_popen: MagicMock) -> None:
        ext = _make_extension()
    ...
    @patch("pyisolate._internal.host.subprocess.Popen")
    def test_sealed_worker_does_not_inject_credential_like_vars(self, mock_popen: MagicMock) -> None:
        credential_pattern = re.compile(r".*(_TOKEN|_SECRET|_KEY|_PASSWORD|_CREDENTIAL)$", re.IGNORECASE)
        ext = _make_extension()
...
class TestSealedWorkerBootstrapGuards:
    @patch("pyisolate._internal.host.subprocess.Popen")
    def test_sealed_worker_snapshot_disables_host_sys_path(self, mock_popen: MagicMock) -> None:
        ext = _make_extension()
